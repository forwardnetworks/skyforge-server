...
- `encore.app/skyforge` (core domain orchestration; auth/session, user scope membership)
...
- `encore.app/jobs/usersync`
  - `RunOnce(ctx)` and `RunForUserScope(ctx, userScopeKey)` style entrypoints.
  - Called by:
