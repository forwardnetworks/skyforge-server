...
class VIOS_vm(vrnetlab.VM):
    ...
    def apply_config(self):
        ...
        # IOSv won't start SSH until RSA keys exist, and key generation fails when the
        # hostname is still the default "Router". Set hostname first and then ensure keys.
        access_cfg = [
            f"hostname {self.hostname}",
            f"username {self.username} privilege 15 secret {self.password}",
            "ip domain name lab",
            # VRNetlab port-forwarding targets 10.0.0.15/24 on Gi0/0. IOSv does not
            # run DHCP by default, so we must ensure Gi0/0 has the expected address.
        ...
        # Best-effort: ensure RSA keys exist so SSH can come up (IOSv requires this).
        # Key generation requires an IP domain-name and must run in config mode.
        try:
            ...
            if not has_keys:
                ...
                res += con.send_configs(["crypto key generate rsa modulus 2048\n"])
