...
func ProcessQueuedTask(ctx context.Context, db *sql.DB, taskID int, deps Deps, log Logger) error {
	...
	if rec, err := taskstore.GetTask(ctx, db, task.ID); err == nil && rec != nil && strings.EqualFold(strings.TrimSpace(rec.Status), "canceled") {
		...
		if task.DeploymentID.Valid && deps.UpdateDeploymentStatus != nil {
			_ = deps.UpdateDeploymentStatus(ctx, task.UserScopeID, task.DeploymentID.String, "canceled", time.Now())
		}
		return nil
	}
	...
	if task.DeploymentID.Valid && deps.UpdateDeploymentStatus != nil {
		_ = deps.UpdateDeploymentStatus(ctx, task.UserScopeID, task.DeploymentID.String, status, time.Now())
	}
	...
	if deps.EnqueueNextDeploymentTask != nil && task.DeploymentID.Valid {
		...
		if userScopeID != "" && deploymentID != "" {
			if nextID, err := taskstore.GetOldestQueuedDeploymentTaskID(ctx, db, userScopeID, deploymentID); err == nil && nextID > 0 {
				ctxKick, cancel := context.WithTimeout(context.Background(), 2*time.Second)
				deps.EnqueueNextDeploymentTask(ctxKick, nextID, userScopeID, deploymentID)
				cancel()
			}
		}
	}
