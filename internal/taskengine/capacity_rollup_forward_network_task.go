...
func (e *Engine) runCapacityRollupForwardNetwork(ctx context.Context, pc *userContext, forwardNetworkID string, collectorConfigID string, taskID int, log Logger) error {
	...
	inv, invErr := e.refreshCapacityInventoryCache(ctx, e.db, client, pc.userScope.ID, nil, forwardNetworkID, log)
	...
	for _, w := range windows {
		threshold := 0.85
		if err := e.rollupInterfaceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "UTILIZATION", "util_ingress", "INGRESS", &threshold, inv, taskID, log); err != nil {
			log.Errorf("interface rollup failed (window=%s type=%s dir=%s): %v", w.Label, "UTILIZATION", "INGRESS", err)
		}
		if err := e.rollupInterfaceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "UTILIZATION", "util_egress", "EGRESS", &threshold, inv, taskID, log); err != nil {
			log.Errorf("interface rollup failed (window=%s type=%s dir=%s): %v", w.Label, "UTILIZATION", "EGRESS", err)
		}

		if err := e.rollupInterfaceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "ERROR", "if_error_ingress", "INGRESS", nil, inv, taskID, log); err != nil {
			log.Errorf("interface rollup failed (window=%s type=%s dir=%s): %v", w.Label, "ERROR", "INGRESS", err)
		}
		if err := e.rollupInterfaceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "ERROR", "if_error_egress", "EGRESS", nil, inv, taskID, log); err != nil {
			log.Errorf("interface rollup failed (window=%s type=%s dir=%s): %v", w.Label, "ERROR", "EGRESS", err)
		}
		if err := e.rollupInterfaceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "PACKET_LOSS", "if_packet_loss_ingress", "INGRESS", nil, inv, taskID, log); err != nil {
			log.Errorf("interface rollup failed (window=%s type=%s dir=%s): %v", w.Label, "PACKET_LOSS", "INGRESS", err)
		}
		if err := e.rollupInterfaceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "PACKET_LOSS", "if_packet_loss_egress", "EGRESS", nil, inv, taskID, log); err != nil {
			log.Errorf("interface rollup failed (window=%s type=%s dir=%s): %v", w.Label, "PACKET_LOSS", "EGRESS", err)
		}

		if err := e.rollupDeviceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "CPU", inv, taskID, log); err != nil {
			log.Errorf("device rollup failed (window=%s type=%s): %v", w.Label, "CPU", err)
		}
		if err := e.rollupDeviceMetric(ctx, client, pc.userScope.ID, nil, forwardNetworkID, periodEnd, w, "MEMORY", inv, taskID, log); err != nil {
