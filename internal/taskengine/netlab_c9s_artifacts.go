...
func storeNetlabC9sArtifacts(ctx context.Context, cfg skyforgecore.Config, spec netlabC9sArtifactsSpec, log Logger) error {
	...
	spec.UserScopeID = strings.TrimSpace(spec.UserScopeID)
	...
	if spec.UserScopeID == "" || spec.TopologyName == "" || spec.Namespace == "" {
		return fmt.Errorf("missing user-scope/topology context")
	}
	...
	if len(spec.TopologyYAML) > 0 {
		...
		_, err := putUserScopeArtifact(ctx, cfg, spec.UserScopeID, prefix+path.Join("clabernetes", "topology.yaml"), spec.TopologyYAML, "application/yaml")
	...
	if len(spec.ClabYAMLRaw) > 0 {
		...
		_, err := putUserScopeArtifact(ctx, cfg, spec.UserScopeID, prefix+path.Join("netlab", "clab.yml"), spec.ClabYAMLRaw, "application/yaml")
		if err != nil && isObjectStoreNotConfigured(err) {
			log.Infof("netlab-c9s artifacts skipped: %v", err)
			return nil
		} else if err != nil {
	...
	if data, ok, err := kubeGetConfigMap(ctx, spec.Namespace, manifestCM); err == nil && ok {
		if raw := strings.TrimSpace(data["manifest.json"]); raw != "" {
			...
			_, err := putUserScopeArtifact(ctx, cfg, spec.UserScopeID, prefix+path.Join("netlab", "manifest.json"), []byte(raw), "application/json")
			if err != nil && isObjectStoreNotConfigured(err) {
				log.Infof("netlab-c9s artifacts skipped: %v", err)
				return nil
			} else if err != nil {
	...
	if spec.TopologyGraph != nil {
		if graphBytes, err := json.Marshal(spec.TopologyGraph); err == nil && len(graphBytes) > 0 {
			...
			_, err := putUserScopeArtifact(ctx, cfg, spec.UserScopeID, prefix+path.Join("clabernetes", "topology.json"), graphBytes, "application/json")
			if err != nil && isObjectStoreNotConfigured(err) {
				log.Infof("netlab-c9s artifacts skipped: %v", err)
				return nil
			} else if err != nil {
	...
	defer cancel()
	if _, err := putUserScopeArtifact(ctxPut, cfg, spec.UserScopeID, prefix+"bundle.tar.gz", tarGz, "application/gzip"); err != nil {
