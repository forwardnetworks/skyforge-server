...
func (e *Engine) giteaClient() *gitea.Client {
	return gitea.New(gitea.Config{
		APIURL:      strings.TrimRight(strings.TrimSpace(e.cfg.UserScopes.GiteaAPIURL), "/"),
		Username:    strings.TrimSpace(e.cfg.UserScopes.GiteaUsername),
		Password:    strings.TrimSpace(e.cfg.UserScopes.GiteaPassword),
...
func (e *Engine) resolveTemplateRepoForUserScope(pc *userContext, source string, customRepo string) (templateRepoRef, error) {
	...
	owner := strings.TrimSpace(pc.userScope.GiteaOwner)
	repo := strings.TrimSpace(pc.userScope.GiteaRepo)
	...
	case "blueprints", "blueprint":
		...
		branch = ""
	case "external":
		if !pc.userScope.AllowExternalTemplateRepos {
			return templateRepoRef{}, fmt.Errorf("external template repos are not enabled for this user scope")
		}
		...
		for i := range pc.userScope.ExternalTemplateRepos {
			if strings.TrimSpace(pc.userScope.ExternalTemplateRepos[i].ID) == repoID {
				found = &pc.userScope.ExternalTemplateRepos[i]
				break
			}
		}
		if found == nil {
	...
	case "custom":
		if !pc.userScope.AllowExternalTemplateRepos {
			return templateRepoRef{}, fmt.Errorf("custom template repos are not enabled for this user scope")
		}
