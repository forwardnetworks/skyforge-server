...
func (e *Engine) DispatchTask(ctx context.Context, task *taskstore.TaskRecord, log taskexec.Logger) (handled bool, err error) {
	...
	case typ == skyforgecore.TaskTypeContextBootstrap:
		return true, e.dispatchOwnerContextBootstrapTask(ctx, task, taskLogAdapter(log))
	...
	case typ == skyforgecore.TaskTypeCapacityRollupForwardNetwork:
		return true, e.dispatchCapacityRollupForwardNetworkTask(ctx, task, taskLogAdapter(log))
	default:
		return false, nil
	}
}
