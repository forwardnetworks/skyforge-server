...
func (e *Engine) runForwardSyncTask(ctx context.Context, pc *userContext, deploymentID string, taskID int, log Logger) error {
	...
	for _, t := range taskTypes {
		...
		latest, err := taskstore.GetLatestDeploymentTask(ctxReq, e.db, pc.userContext.ID, dep.ID, t)
		cancel()
	...
	defer cancel()
	raw, err := readUserContextArtifact(ctxRead, e.cfg, pc.userContext.ID, topoKey, 2<<20)
