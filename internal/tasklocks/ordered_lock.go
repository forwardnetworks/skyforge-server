...
// per-deployment/per-user-context ordering by waiting until the task is the oldest
...
func AcquireOrderedTaskLock(ctx context.Context, db *sql.DB, task *taskstore.TaskRecord) (unlock func(), err error) {
	...
	if deploymentID != "" {
		lockKey := pglocks.DeploymentAdvisoryLockKey(userContextID, deploymentID)
		for {
			...
			oldestQueuedID, err := taskstore.GetOldestQueuedDeploymentTaskID(ctx, db, userContextID, deploymentID)
			if err != nil {
	...
	lockKey := pglocks.WorkspaceAdvisoryLockKey(userContextID)
	for {
		lock, ok, err := pglocks.TryAdvisoryLock(ctx, db, lockKey)
		if err != nil {
			rlog.Error("user-context lock error", "err", err)
			if err := sleep(); err != nil {
				return func() {}, err
			}
			continue
		...
		oldestQueuedID, err := taskstore.GetOldestQueuedUserContextTaskID(ctx, db, userContextID)
		if err != nil {
			_ = lock.Unlock(context.Background())
			rlog.Error("user-context queue check error", "err", err)
			if err := sleep(); err != nil {
				return func() {}, err
			}
			continue
