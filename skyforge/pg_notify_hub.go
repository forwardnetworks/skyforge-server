...
const (
	...
	pgNotifyUserContextsChannel  = "skyforge_user_contexts_updates"
...
func (h *pgNotifyHub) listenOnPGX(ctx context.Context, conn *pgx.Conn) error {
	...
	if _, err := conn.Exec(ctx, "LISTEN "+pgNotifyUserContextsChannel); err != nil {
		return err
	}
...
func notifyUserContextsUpdatePG(ctx context.Context, db *sql.DB, payload string) error {
	...
	ctxReq, cancel := context.WithTimeout(ctx, 1*time.Second)
	defer cancel()
	_, err := db.ExecContext(ctxReq, "SELECT pg_notify($1, $2)", pgNotifyUserContextsChannel, payload)
	return err
}

func notifyDeploymentEventPG(ctx context.Context, db *sql.DB, userContextID, deploymentID string) error {
	...
	ctxReq, cancel := context.WithTimeout(ctx, 1*time.Second)
	defer cancel()
	payload := userContextID + ":" + deploymentID
...
func waitForUserContextsUpdateSignal(ctx context.Context, db *sql.DB, username string) bool {
	username = strings.ToLower(strings.TrimSpace(username))
	if username == "" {
		return false
	}
	...
	for {
		...
		case n, ok := <-ch:
			...
			if n.Channel != pgNotifyUserContextsChannel {
				continue
			}
...
func waitForDeploymentEventSignal(ctx context.Context, db *sql.DB, userContextID, deploymentID string) bool {
	userContextID = strings.TrimSpace(userContextID)
	deploymentID = strings.TrimSpace(deploymentID)
	if userContextID == "" || deploymentID == "" {
		return false
	}
	payloadWant := userContextID + ":" + deploymentID
	hub := ensurePGNotifyHub(db)
	ch := hub.subscribe(ctx)
	for {
		select {
