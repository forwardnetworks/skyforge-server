...
SELECT id::text, workspace_id, forward_network_id, kind, pack_id, COALESCE(title_template,''), snapshot_id,
 ...
 LIMIT 20`)
	...
	for rows.Next() {
		var p duePreset
		if err := rows.Scan(&p.id, &p.userContextID, &p.forwardNetworkID, &p.kind, &p.packID, &p.titleTemplate, &p.snapshotID,
			&p.checksJSON, &p.queryOptionsJSON, &p.maxPerCheck, &p.maxTotal, &p.intervalMinutes, &p.ownerUsername); err != nil {
			continue
		}
		p.id = strings.TrimSpace(p.id)
		p.userContextID = strings.TrimSpace(p.userContextID)
		...
		if p.id == "" || p.userContextID == "" || p.forwardNetworkID == "" || p.ownerUsername == "" {
			continue
		}
		due = append(due, p)
	}
...
func runPolicyReportPresetInternal(ctx context.Context, db *sql.DB, sessionSecret string, p struct {
	...
	client, err := policyReportsForwardClientFor(ctx, db, sessionSecret, p.userContextID, p.ownerUsername, p.forwardNetworkID)
	if err != nil {
		_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", startedAt, p.intervalMinutes, err.Error())
		return err
	}
	...
	if p.kind == "PATHS" {
		...
		if len(specs) == 0 {
			_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", startedAt, p.intervalMinutes, "preset checks empty")
		...
		if spec == nil {
			_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", startedAt, p.intervalMinutes, "missing paths-enforcement-bypass check")
		...
		if strings.TrimSpace(checkID) == "" {
			_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", startedAt, p.intervalMinutes, "failed to compute suite check id")
		...
		if err := persistPolicyReportRun(ctx, db, &run, checks, findings, resolveChecks); err != nil {
			_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", finishedAt, p.intervalMinutes, err.Error())
			return err
		}

		_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, run.ID, finishedAt, p.intervalMinutes, "")
		return nil
	}
	...
	if p.kind == "PACK" {
		...
		if err != nil || packs == nil {
			_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", startedAt, p.intervalMinutes, "packs unavailable")
		...
		if pack == nil {
			_ = bumpPresetSchedule(ctx, db, p.userContextID, p.id, "", startedAt, p.intervalMinutes, "pack not found")
...
func bumpPresetSchedule(ctx context.Context, db *sql.DB, userContextID, presetID, runID string, finishedAt time.Time, intervalMinutes int, lastErr string) error {
	if db == nil {
		return nil
	}
...
`, strings.TrimSpace(runID), finishedAt, strings.TrimSpace(lastErr), next, userContextID, presetID)
	return nil
}
