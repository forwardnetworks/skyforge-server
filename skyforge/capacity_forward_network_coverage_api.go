...
// GetUserForwardNetworkCapacityCoverage returns collection coverage / data-quality counters.
//
...
func (s *Service) GetUserForwardNetworkCapacityCoverage(ctx context.Context, id, networkRef string) (*ForwardNetworkCapacityCoverageResponse, error) {
	...
	asOfInv, _, devices, ifaces, _, _, _, _, err := loadLatestCapacityInventoryForForwardNetwork(ctx, s.db, pc.context.ID, net.ForwardNetworkID)
	if err != nil {
	...
	asOfRollups, ifaceRows, devRows, withSamples, err := loadForwardNetworkRollupCoverage(ctx, s.db, pc.context.ID, net.ForwardNetworkID)
	if err != nil {
...
func loadForwardNetworkRollupCoverage(ctx context.Context, db *sql.DB, ownerID, forwardNetworkID string) (asOf time.Time, ifaceRows int, devRows int, withSamples int, err error) {
	if db == nil {
...
FROM sf_capacity_rollups
WHERE owner_username=$1 AND forward_network_id=$2 AND deployment_id IS NULL AND period_end=$3`, ownerID, forwardNetworkID, asOf)
	if err != nil {
		return time.Time{}, 0, 0, 0, err
	}
	defer rows.Close()
