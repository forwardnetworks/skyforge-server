...
func (s *Service) mergeDeploymentEnvironment(ctx context.Context, userContextID, username string, cfgAny map[string]any) (map[string]string, error) {
	if s.db == nil {
	...
	if len(groupIDs) > 0 {
		...
		if scope == "user" {
			...
			groupEnv, err = loadUserContextVariableGroupsByID(ctx, s.db, userContextID, groupIDs)
		}
		if err != nil {
			return nil, err
		}
...
func loadUserContextVariableGroupsByID(ctx context.Context, db *sql.DB, userContextID string, groupIDs []int) (map[string]string, error) {
	if len(groupIDs) == 0 {
		return map[string]string{}, nil
	}
	placeholders := make([]string, 0, len(groupIDs))
	args := make([]any, 0, len(groupIDs)+1)
	args = append(args, userContextID)
