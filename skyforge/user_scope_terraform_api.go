...
type UserScopeTerraformTemplatesResponse struct {
...
type UserScopeTerraformTemplatesRequest struct {
...
// GetUserScopeTerraformTemplates lists Terraform template directories for a user scope.
//
//encore:api auth method=GET path=/api/users/:id/terraform/templates
func (s *Service) GetUserScopeTerraformTemplates(ctx context.Context, id string, req *UserScopeTerraformTemplatesRequest) (*UserScopeTerraformTemplatesResponse, error) {
	...
	case "blueprints", "blueprint", "external", "custom":
		...
		// A user scope repo syncs that same content under `blueprints/terraform/`.
		dir := "terraform"
		if req != nil {
		...
		_ = ctx
		return &UserScopeTerraformTemplatesResponse{
