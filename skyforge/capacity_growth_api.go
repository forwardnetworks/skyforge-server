...
// GetUserDeploymentCapacityGrowth compares the latest rollup bucket to an earlier one and returns deltas.
//
// Intended for “top growers” views (week-over-week, day-over-day) without pulling Forward time series.
func (s *Service) GetUserDeploymentCapacityGrowth(ctx context.Context, id, deploymentID string, q *DeploymentCapacityGrowthQuery) (*DeploymentCapacityGrowthResponse, error) {
	...
	_, _, _, err = s.requireDeploymentForwardNetwork(ctx, pc.context.ID, deploymentID)
	if err != nil {
		return nil, err
	}
	if q == nil {
	...
	asOf, compareAsOf, rows, err := loadCapacityGrowth(ctx, s.db, pc.context.ID, deploymentID, metric, window, objectType, time.Duration(compareHours)*time.Hour)
	if err != nil {
...
func loadCapacityGrowth(ctx context.Context, db *sql.DB, ownerID, deploymentID, metric, window, objectType string, compareDur time.Duration) (asOf time.Time, compareAsOf time.Time, out []CapacityGrowthRow, err error) {
	if db == nil {
...
WHERE owner_username=$1 AND deployment_id=$2 AND metric=$3 AND window=$4`,
		ownerID, deploymentID, metric, window).Scan(&asOf); err != nil {
	...
	if err := db.QueryRowContext(ctxReq, `SELECT COALESCE(MAX(period_end), 'epoch'::timestamptz)
FROM sf_capacity_rollups
WHERE owner_username=$1 AND deployment_id=$2 AND metric=$3 AND window=$4 AND period_end <= $5`,
		ownerID, deploymentID, metric, window, target).Scan(&compareAsOf); err != nil {
	...
	loadBucket := func(ts time.Time) ([]CapacityRollupRow, error) {
		...
		args := []any{ownerID, deploymentID, metric, window, ts}
