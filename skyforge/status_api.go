...
type StatusSummaryResponse struct {
	...
	UsersTotal        int `json:"usersTotal,omitempty"`
...
func countScopes(ctx context.Context, db *sql.DB) (int, error) {
	if db == nil {
		return 0, nil
	}
	var count int
	if err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM sf_owner_contexts`).Scan(&count); err != nil {
		return 0, err
	}
	return count, nil
}
...
//
// This is designed for an unauthenticated landing page and must not leak user identifiers.
//
...
func (s *Service) StatusSummary(ctx context.Context) (*StatusSummaryResponse, error) {
	...
	if total, err := countScopes(ctx, s.db); err == nil {
		resp.UsersTotal = total
		resp.UsersTotal = total
	}
