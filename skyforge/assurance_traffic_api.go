...
// PostUserContextForwardNetworkAssuranceTrafficSeeds discovers seed endpoints via NQE and produces a starter demand set.
func (s *Service) PostUserContextForwardNetworkAssuranceTrafficSeeds(ctx context.Context, id, networkRef string, req *AssuranceTrafficSeedRequest) (*AssuranceTrafficSeedResponse, error) {
...
// PostUserContextForwardNetworkAssuranceTrafficEvaluate evaluates demands by calling Forward paths-bulk and overlaying
// capacity/security constraints. This is a demo-first "rank + explain" view (no prediction/simulation).
func (s *Service) PostUserContextForwardNetworkAssuranceTrafficEvaluate(ctx context.Context, id, networkRef string, req *AssuranceTrafficEvaluateRequest) (*AssuranceTrafficEvaluateResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	asOfTS, rollups, rollErr := loadLatestCapacityRollupsForForwardNetwork(ctx, s.db, pc.claims.Username, pc.userContext.ID, net.ForwardNetworkID)
	if rollErr != nil {
