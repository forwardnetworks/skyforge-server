...
type UserScopeDeploymentListResponse struct {
	UserScopeID string                 `json:"userId"`
	Deployments []*UserScopeDeployment `json:"deployments"`
}

type UserScopeDeploymentCreateRequest struct {
	Name   string  `json:"name"`
	Type   string  `json:"type"`
	Config JSONMap `json:"config,omitempty"`
}

type UserScopeDeploymentUpdateRequest struct {
	Name   string  `json:"name,omitempty"`
	Config JSONMap `json:"config,omitempty"`
}

type UserScopeDeploymentActionResponse struct {
	UserScopeID string               `json:"userId"`
	Deployment  *UserScopeDeployment `json:"deployment"`
	Run         JSONMap              `json:"run,omitempty"`
}

type UserScopeDeploymentDeleteRequest struct {
...
type UserScopeDeploymentInfoResponse struct {
	UserScopeID  string               `json:"userId"`
	Deployment   *UserScopeDeployment `json:"deployment"`
...
//
//encore:api auth method=GET path=/api/users/:id/deployments
func (s *Service) ListUserScopeDeployments(ctx context.Context, id string) (*UserScopeDeploymentListResponse, error) {
...
FROM sf_deployments
WHERE user_id=$1
ORDER BY updated_at DESC`, pc.userScope.ID)
	if err != nil {
	...
	out := make([]*UserScopeDeployment, 0, 16)
	refresh := make([]*UserScopeDeployment, 0, 4)
	for rows.Next() {
		var (
		...
		rec.UserScopeID = pc.userScope.ID
		...
		rec.UpdatedAt = updatedAt.UTC().Format(time.RFC3339)
		{
			...
			summary, err := getDeploymentQueueSummary(qctx, s.db, pc.userScope.ID, rec.ID)
			cancel()
	...
	if len(refresh) > 0 {
		for _, dep := range refresh {
			...
			if err := s.updateDeploymentStatus(ctx, pc.userScope.ID, dep.ID, status, finishedAt); err != nil {
	...
	return &UserScopeDeploymentListResponse{UserScopeID: pc.userScope.ID, Deployments: out}, nil
}

// CreateUserScopeDeployment creates a deployment definition for a user scope.
//
//encore:api auth method=POST path=/api/users/:id/deployments
func (s *Service) CreateUserScopeDeployment(ctx context.Context, id string, req *UserScopeDeploymentCreateRequest) (*UserScopeDeployment, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	case "netlab-c9s":
		...
		cfgAny["templatesDir"] = templatesDir
		cfgAny["template"] = template
		cfgAny["labName"] = containerlabLabName(pc.userScope.Slug, name)
	case "eve_ng":
  ...
  id, user_id, name, type, config, created_by, last_status
) VALUES ($1,$2,$3,$4,$5,$6,$7)`, deploymentID, pc.userScope.ID, name, typ, cfgBytes, pc.claims.Username, initialStatus)
	if err != nil {
	...
	if enabled, ok := cfgAny["forwardEnabled"].(bool); ok && enabled {
		...
		if task, err := createTaskAllowActive(ctx, s.db, pc.userScope.ID, &deploymentID, "forward-init", msg, pc.claims.Username, meta); err != nil {
...
//
//encore:api auth method=PUT path=/api/users/:id/deployments/:deploymentID
func (s *Service) UpdateUserScopeDeployment(ctx context.Context, id, deploymentID string, req *UserScopeDeploymentUpdateRequest) (*UserScopeDeployment, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	query := fmt.Sprintf("UPDATE sf_deployments SET %s WHERE user_id=$%d AND id=$%d", strings.Join(fields, ", "), len(args)-1, len(args))
	ctx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()
...
//
//encore:api auth method=DELETE path=/api/users/:id/deployments/:deploymentID
func (s *Service) DeleteUserScopeDeployment(ctx context.Context, id, deploymentID string, req *UserScopeDeploymentDeleteRequest) (*UserScopeDeploymentActionResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
...
WHERE user_id=$1 AND deployment_id=$2 AND status IN ('queued','running')`, pc.userScope.ID, existing.ID).Scan(&n)
		if n > 0 {
	...
	if existing.Type == "netlab" {
		...
		if strings.TrimSpace(netlabServer) != "" {
			_, err := s.RunUserScopeNetlab(ctx, id, &UserScopeNetlabRunRequest{
	...
	if existing.Type == "netlab-c9s" || existing.Type == "clabernetes" || existing.Type == "eve_ng" {
		_, err := s.RunUserScopeDeploymentAction(ctx, id, deploymentID, &UserScopeDeploymentOpRequest{Action: "destroy"})
		if err != nil {
	...
	if req != nil && (req.ForwardDelete || req.ForwardDeleteCamel) {
		...
		if raw, ok := cfgAny[forwardNetworkIDKey]; ok {
			...
			if networkID != "" {
				// Forward config is user-scoped (Collector sidebar) and must come from the
				// deployment-selected credential profile.
				...
				if forwardCfg == nil {
					return nil, errs.B().Code(errs.FailedPrecondition).Msg("Forward collector credential profile is required (cannot delete Forward network)").Err()
				}
	...
	ctx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()
	res, err := s.db.ExecContext(ctx, `DELETE FROM sf_deployments WHERE user_id=$1 AND id=$2`, pc.userScope.ID, deploymentID)
	if err != nil {
	...
	return &UserScopeDeploymentActionResponse{UserScopeID: pc.userScope.ID, Deployment: existing}, nil
}

type UserScopeDeploymentStartRequest struct {
	Action string `json:"action,omitempty"` // used for terraform (apply/destroy)
}

type UserScopeDeploymentOpRequest struct {
	Action string `json:"action,omitempty"` // create, start, stop, destroy, export
}

// RunUserScopeDeploymentAction runs a deployment operation with consistent UX verbs.
//
//encore:api auth method=POST path=/api/users/:id/deployments/:deploymentID/action
func (s *Service) RunUserScopeDeploymentAction(ctx context.Context, id, deploymentID string, req *UserScopeDeploymentOpRequest) (*UserScopeDeploymentActionResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	if req == nil {
		req = &UserScopeDeploymentOpRequest{}
	}
	...
	envMap, err := s.mergeDeploymentEnvironment(ctx, pc.userScope.ID, user.Username, cfgAny)
	if err != nil {
		return nil, err
	}
	envJSON := JSONMap{}
	...
	case "terraform":
		...
		switch op {
		case "create":
			run, err = s.RunUserScopeTerraformApply(ctx, id, &UserScopeTerraformApplyParams{
				Confirm:        "true",
				Cloud:          cloud,
				Action:         "apply",
	...
	case "netlab":
		...
		run, err = s.RunUserScopeNetlab(ctx, id, &UserScopeNetlabRunRequest{
			Message:          message,
			GitBranch:        branch,
			Environment:      envJSON,
			Action:           netlabAction,
	...
	case "eve_ng":
		...
		run, err = s.RunUserScopeEveNg(ctx, id, &UserScopeEveNgRunRequest{
	...
	case "containerlab":
		...
		run, err = s.RunUserScopeContainerlab(ctx, id, &UserScopeContainerlabRunRequest{
		...
		if strings.TrimSpace(labName) == "" {
			cfgAny["labName"] = containerlabLabName(pc.userScope.Slug, dep.Name)
		}
	case "clabernetes":
		templateSource, _ := cfgAny["templateSource"].(string)
		templateRepo, _ := cfgAny["templateRepo"].(string)
	...
	updated, err := s.touchDeploymentFromRun(ctx, pc.userScope.ID, deploymentID, cfgJSON, run)
	if err != nil {
		log.Printf("deployments touch: %v", err)
		updated = dep
	}

	resp := &UserScopeDeploymentActionResponse{UserScopeID: pc.userScope.ID, Deployment: updated}
	if run != nil {
		resp.Run = run.Task
	}
	return resp, nil
...
//
//encore:api auth method=GET path=/api/users/:id/deployments/:deploymentID/info
func (s *Service) GetUserScopeDeploymentInfo(ctx context.Context, id, deploymentID string) (*UserScopeDeploymentInfoResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	resp := &UserScopeDeploymentInfoResponse{
	...
	case "netlab":
		...
		payload := map[string]any{
			...
			"userScope":     strings.TrimSpace(pc.userScope.Slug),
			"deployment":    strings.TrimSpace(dep.Name),
		...
		return resp, nil
	case "terraform":
		stateKey := strings.TrimSpace(pc.userScope.TerraformStateKey)
		if stateKey == "" {
...
//
//encore:api auth method=POST path=/api/users/:id/deployments/:deploymentID/netlab/connect
...
// GetUserScopeDeploymentNetlabGraph returns a rendered netlab topology graph for a deployment.
//
//encore:api auth method=GET path=/api/users/:id/deployments/:deploymentID/netlab-graph
func (s *Service) GetUserScopeDeploymentNetlabGraph(ctx context.Context, id, deploymentID string) (*NetlabGraphResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
...
//
//encore:api auth method=POST path=/api/users/:id/deployments/:deploymentID/start
func (s *Service) StartUserScopeDeployment(ctx context.Context, id, deploymentID string, req *UserScopeDeploymentStartRequest) (*UserScopeDeploymentActionResponse, error) {
	return s.runDeployment(ctx, id, deploymentID, req, "start")
}

// DestroyUserScopeDeployment triggers a destructive run (destroy) for a deployment.
//
//encore:api auth method=POST path=/api/users/:id/deployments/:deploymentID/destroy
func (s *Service) DestroyUserScopeDeployment(ctx context.Context, id, deploymentID string) (*UserScopeDeploymentActionResponse, error) {
	return s.runDeployment(ctx, id, deploymentID, &UserScopeDeploymentStartRequest{Action: "destroy"}, "destroy")
}

// StopUserScopeDeployment attempts to stop the most recent task for this deployment.
//
//encore:api auth method=POST path=/api/users/:id/deployments/:deploymentID/stop
func (s *Service) StopUserScopeDeployment(ctx context.Context, id, deploymentID string) (*UserScopeDeploymentActionResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	var task *TaskRecord
	if active, err := getActiveDeploymentTask(ctx, s.db, pc.userScope.ID, dep.ID); err == nil && active != nil {
		task = active
	...
	if task == nil {
		return &UserScopeDeploymentActionResponse{UserScopeID: pc.userScope.ID, Deployment: dep}, nil
	}
	...
	now := time.Now().UTC()
	if err := s.updateDeploymentStatus(ctx, pc.userScope.ID, dep.ID, "canceled", &now); err != nil {
		log.Printf("deployment stop update: %v", err)
	}
	return &UserScopeDeploymentActionResponse{UserScopeID: pc.userScope.ID, Deployment: dep}, nil
}

func (s *Service) runDeployment(ctx context.Context, id, deploymentID string, req *UserScopeDeploymentStartRequest, mode string) (*UserScopeDeploymentActionResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	updated, err := s.touchDeploymentFromRun(ctx, pc.userScope.ID, deploymentID, cfgOut, run)
	if err != nil {
		log.Printf("deployments touch: %v", err)
	}
...
func (s *Service) touchDeploymentFromRun(ctx context.Context, userScopeID, deploymentID string, cfg JSONMap, run *UserScopeRunResponse) (*UserScopeDeployment, error) {
	if s == nil || s.db == nil {
		return nil, errs.B().Code(errs.Unavailable).Msg("database unavailable").Err()
	}
	taskUserScopeID := 0
...
func (s *Service) updateDeploymentStatus(ctx context.Context, userScopeID, deploymentID string, status string, finishedAt *time.Time) error {
	return taskstore.UpdateDeploymentStatus(ctx, s.db, userScopeID, deploymentID, status, finishedAt)
}

func (s *Service) getLatestDeploymentByType(ctx context.Context, userScopeID, depType string) (*UserScopeDeployment, error) {
	if s.db == nil {
		return nil, errs.B().Code(errs.Unavailable).Msg("database unavailable").Err()
	}
	ctx, cancel := context.WithTimeout(ctx, 2*time.Second)
	...
	err := s.db.QueryRowContext(ctx, `SELECT id FROM sf_deployments WHERE user_id=$1 AND type=$2 ORDER BY updated_at DESC LIMIT 1`, userScopeID, depType).Scan(&deploymentID)
	if err != nil {
...
func (s *Service) getUserScopeDeployment(ctx context.Context, userScopeID, deploymentID string) (*UserScopeDeployment, error) {
  ...
  last_task_user_id, last_task_id, last_status, last_started_at, last_finished_at
FROM sf_deployments
WHERE user_id=$1 AND id=$2`, userScopeID, deploymentID).Scan(
		&rec.ID,
		&rec.Name,
		&rec.Type,
		&raw,
	...
	rec.UserScopeID = userScopeID
	rec.CreatedAt = createdAt.UTC().Format(time.RFC3339)
	rec.UpdatedAt = updatedAt.UTC().Format(time.RFC3339)
	{
		qctx, cancel := context.WithTimeout(context.Background(), 250*time.Millisecond)
		summary, err := getDeploymentQueueSummary(qctx, s.db, userScopeID, rec.ID)
		cancel()
		if err == nil && summary != nil {
			if summary.ActiveTaskID > 0 {
				rec.ActiveTaskID = &summary.ActiveTaskID
