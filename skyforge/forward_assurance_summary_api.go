...
`, strings.TrimSpace(sum.UserContextID), strings.TrimSpace(sum.ForwardNetworkID), strings.TrimSpace(sum.NetworkRef), strings.TrimSpace(sum.Snapshot.SnapshotID), string(b))
	return err
}
...
func loadLatestForwardAssuranceSummary(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, networkRef string) (ForwardAssuranceSummaryResponse, time.Time, bool, error) {
	if db == nil {
...
LIMIT 1
`, strings.TrimSpace(userContextID), strings.TrimSpace(forwardNetworkID), strings.TrimSpace(networkRef)).Scan(&generatedAt, &raw)
	if err != nil {
...
func shouldStoreForwardAssuranceSummary(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, networkRef string, minAge time.Duration) bool {
	if db == nil {
		return false
	}
	ctxQ, cancel := context.WithTimeout(ctx, 2*time.Second)
...
FROM sf_forward_assurance_summaries
WHERE workspace_id=$1 AND forward_network_id=$2 AND network_ref=$3
`, strings.TrimSpace(userContextID), strings.TrimSpace(forwardNetworkID), strings.TrimSpace(networkRef)).Scan(&newest)
	if err != nil {
		return false
	}
...
// GetUserContextForwardNetworkAssuranceSummary returns a demo-oriented assurance summary computed from live Forward calls.
func (s *Service) GetUserContextForwardNetworkAssuranceSummary(ctx context.Context, id, networkRef string, params *ForwardAssuranceSummaryParams) (*ForwardAssuranceSummaryResponse, error) {
	...
	if !forceFresh && s.db != nil {
		cached, generatedAt, ok, err := loadLatestForwardAssuranceSummary(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, net.ID)
	...
	var capErr error
	capAsOf, capRows, capErr = loadLatestCapacityRollupsForForwardNetwork(ctx, s.db, pc.claims.Username, pc.userContext.ID, net.ForwardNetworkID)

	sum := computeForwardAssuranceSummaryFromLive(time.Now(), pc.userContext.ID, net.ID, net.ForwardNetworkID, snap, haveSnap, met, haveMet, vul, haveVul, capAsOf, capRows, capErr)
	...
	if s.db != nil && shouldStoreForwardAssuranceSummary(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, net.ID, 2*time.Minute) {
...
// RefreshUserContextForwardNetworkAssurance recomputes the live summary, stores it in history (best-effort), and returns it.
func (s *Service) RefreshUserContextForwardNetworkAssurance(ctx context.Context, id, networkRef string) (*ForwardAssuranceSummaryResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	sum, err := s.GetUserContextForwardNetworkAssuranceSummary(ctx, id, networkRef, &ForwardAssuranceSummaryParams{Fresh: "1"})
	if err != nil {
		return nil, err
	}
	...
	s.indexElasticAsync(pc.claims.Username, "assurance", time.Now().UTC(), map[string]any{
		...
		"workspace_id":       sum.UserContextID,
...
// SeedUserContextForwardNetworkAssuranceDemo inserts a small set of synthetic events for demo purposes.
// Admin-only. It also claims a test CIDR for the current admin user so syslog counts show up.
func (s *Service) SeedUserContextForwardNetworkAssuranceDemo(ctx context.Context, id, networkRef string) (*ForwardAssuranceDemoSeedResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	if _, err := resolveUserForwardNetwork(ctx, s.db, pc.userContext.ID, pc.claims.Username, networkRef); err != nil {
		return nil, err
	}
...
// ListUserContextForwardNetworkAssuranceSummaryHistory returns recent stored summaries (if enabled via migrations).
func (s *Service) ListUserContextForwardNetworkAssuranceSummaryHistory(ctx context.Context, id, networkRef string, params *ForwardAssuranceHistoryParams) (*ForwardAssuranceHistoryResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
...
ORDER BY generated_at DESC, id DESC
LIMIT $4
`, pc.userContext.ID, net.ForwardNetworkID, net.ID, limit)
	if err != nil {
		if isMissingDBRelation(err) {
			return &ForwardAssuranceHistoryResponse{Items: []ForwardAssuranceHistoryItem{}}, nil
		}
