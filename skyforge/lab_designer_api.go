...
type CreateContainerlabDeploymentFromYAMLRequest struct {
	...
	// If omitted, we fall back to the user-scope default.
...
type CreateContainerlabDeploymentFromYAMLResponse struct {
	...
	Deployment  *UserScopeDeployment `json:"deployment,omitempty"`
...
// CreateContainerlabDeploymentFromYAML persists a containerlab topology YAML into the user-scope repo,
...
func (s *Service) CreateContainerlabDeploymentFromYAML(ctx context.Context, id string, req *CreateContainerlabDeploymentFromYAMLRequest) (*CreateContainerlabDeploymentFromYAMLResponse, error) {
	...
	commitMsg := fmt.Sprintf("lab: add containerlab topology (%s)", name)
	{
		...
		defer cancel()
		_ = ctxWrite
		if err := ensureGiteaFile(s.cfg, pc.userScope.GiteaOwner, pc.userScope.GiteaRepo, filePath, content, commitMsg, branch, pc.claims); err != nil {
			return nil, errs.B().Code(errs.Unavailable).Msg("failed to write topology into user-scope repo").Err()
		}
	}
	...
	dep, err := s.CreateUserScopeDeployment(ctx, id, &UserScopeDeploymentCreateRequest{
		Name:   name,
		Type:   "containerlab",
		Config: cfg,
	})
	...
	actionResp, err := s.RunUserScopeDeploymentAction(ctx, id, dep.ID, &UserScopeDeploymentOpRequest{Action: "create"})
	if err != nil {
		return nil, err
	}
...
// CreateClabernetesDeploymentFromYAML persists a containerlab topology YAML into the user-scope repo,
...
// SaveContainerlabTopologyYAML writes a containerlab topology YAML into the user's user-scope repo so it can be
...
// CreateClabernetesDeploymentFromTemplate creates a clabernetes deployment pointing at an existing user-scope template YAML
// (no YAML commit step).
//
...
func (s *Service) CreateClabernetesDeploymentFromTemplate(ctx context.Context, id string, req *CreateDeploymentFromTemplateRequest) (*CreateDeploymentFromTemplateResponse, error) {
	...
	filePath := path.Join(templatesDir, template)
	{
		body, err := readGiteaFileBytes(s.cfg, pc.userScope.GiteaOwner, pc.userScope.GiteaRepo, filePath, branch)
		if err != nil {
			return nil, errs.B().Code(errs.NotFound).Msg("template not found in user-scope repo").Err()
		}
		var parsed map[string]any
...
// CreateContainerlabDeploymentFromTemplate creates a containerlab (BYOS) deployment pointing at an existing user-scope template YAML
// (no YAML commit step).
//
