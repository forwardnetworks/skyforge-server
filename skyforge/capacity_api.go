...
func (s *Service) requireDeploymentForwardNetwork(ctx context.Context, userContextID, deploymentID string) (dep *UserDeployment, cfgAny map[string]any, forwardNetworkID string, err error) {
	if s == nil || s.db == nil {
		return nil, nil, "", errs.B().Code(errs.Unavailable).Msg("database unavailable").Err()
	}
	dep, err = s.getUserDeployment(ctx, userContextID, deploymentID)
	if err != nil {
		return nil, nil, "", err
	}
	if dep == nil {
...
//
//encore:api auth method=GET path=/api/user-contexts/:id/deployments/:deploymentID/capacity/summary
func (s *Service) GetUserDeploymentCapacitySummary(ctx context.Context, id, deploymentID string) (*DeploymentCapacitySummaryResponse, error) {
	...
	_, _, forwardNetworkID, err := s.requireDeploymentForwardNetwork(ctx, pc.userContext.ID, deploymentID)
	if err != nil {
		return nil, err
	}

	periodEnd, rows, err := loadLatestCapacityRollups(ctx, s.db, pc.userContext.ID, deploymentID)
	if err != nil {
...
//
//encore:api auth method=POST path=/api/user-contexts/:id/deployments/:deploymentID/capacity/rollups/refresh
func (s *Service) RefreshUserDeploymentCapacityRollups(ctx context.Context, id, deploymentID string, req *capacityRollupRefreshRequest) (*DeploymentCapacityRefreshResponse, error) {
	...
	_, cfgAny, _, err := s.requireDeploymentForwardNetwork(ctx, pc.userContext.ID, deploymentID)
	if err != nil {
		return nil, err
	}
	...
	task, err := createTaskAllowActive(ctx, s.db, pc.userContext.ID, &deploymentID, "capacity-rollup", msg, pc.claims.Username, meta)
	if err != nil {
...
//
//encore:api auth method=GET path=/api/user-contexts/:id/deployments/:deploymentID/capacity/perf/interface-metrics
func (s *Service) GetUserDeploymentCapacityInterfaceMetrics(ctx context.Context, id, deploymentID string, q *CapacityInterfaceMetricsQuery) (*CapacityPerfProxyResponse, error) {
	...
	_, cfgAny, forwardNetworkID, err := s.requireDeploymentForwardNetwork(ctx, pc.userContext.ID, deploymentID)
	if err != nil {
		return nil, err
	}
...
//
//encore:api auth method=POST path=/api/user-contexts/:id/deployments/:deploymentID/capacity/perf/interface-metrics-history
...
//
//encore:api auth method=GET path=/api/user-contexts/:id/deployments/:deploymentID/capacity/perf/device-metrics
...
//
//encore:api auth method=POST path=/api/user-contexts/:id/deployments/:deploymentID/capacity/perf/device-metrics-history
...
//
//encore:api auth method=GET path=/api/user-contexts/:id/deployments/:deploymentID/capacity/perf/unhealthy-devices
...
//
//encore:api auth method=POST path=/api/user-contexts/:id/deployments/:deploymentID/capacity/perf/unhealthy-interfaces
...
func loadLatestCapacityRollups(ctx context.Context, db *sql.DB, userContextID, deploymentID string) (time.Time, []CapacityRollupRow, error) {
	if db == nil {
...
WHERE workspace_id=$1 AND deployment_id=$2`, userContextID, deploymentID).Scan(&periodEnd)
	if err != nil {
...
ORDER BY metric, window_label, object_type, object_id`, userContextID, deploymentID, periodEnd)
	if err != nil {
		return time.Time{}, nil, err
	}
	defer rows.Close()
