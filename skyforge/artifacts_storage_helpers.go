...
func artifactBasePrefix(userContextID string) string {
	userContextID = strings.TrimSpace(userContextID)
	if userContextID == "" {
		return "artifacts/"
	}
	return fmt.Sprintf("artifacts/%s/", userContextID)
}

func listArtifactEntries(ctx context.Context, cfg Config, userContextID, prefix string, limit int) ([]storageObjectSummary, error) {
	...
	if c, err := objectStoreClientFor(cfg); err == nil && c != nil {
		basePrefix := artifactBasePrefix(userContextID)
...
func artifactAttrs(ctx context.Context, userContextID, key string) (*objects.ObjectAttrs, error) {
	objectName := artifactObjectName(userContextID, key)
	bucket := artifactsBucket()
...
func readUserContextArtifact(ctx context.Context, cfg Config, userContextID, key string, maxBytes int) ([]byte, error) {
	if c, err := objectStoreClientFor(cfg); err == nil && c != nil {
		objectKey := artifactObjectName(userContextID, key)
...
func deleteUserContextArtifacts(ctx context.Context, cfg Config, userContextID string) error {
	if c, err := objectStoreClientFor(cfg); err == nil && c != nil {
		return c.DeletePrefix(ctx, artifactsBucketName, artifactBasePrefix(userContextID))
	} else if err != nil {
		log.Printf("object store client unavailable (falling back to encore objects): %v", err)
	}

	prefix := artifactBasePrefix(userContextID)
