...
func artifactBasePrefix(ownerID string) string {
	ownerID = strings.TrimSpace(ownerID)
	if ownerID == "" {
		return "artifacts/"
	}
	return fmt.Sprintf("artifacts/%s/", ownerID)
}

func listArtifactEntries(ctx context.Context, cfg Config, ownerID, prefix string, limit int) ([]storageObjectSummary, error) {
	...
	if c, err := objectStoreClientFor(cfg); err == nil && c != nil {
		basePrefix := artifactBasePrefix(ownerID)
...
func artifactAttrs(ctx context.Context, ownerID, key string) (*objects.ObjectAttrs, error) {
	objectName := artifactObjectName(ownerID, key)
	bucket := artifactsBucket()
...
func readUserArtifact(ctx context.Context, cfg Config, ownerID, key string, maxBytes int) ([]byte, error) {
	if c, err := objectStoreClientFor(cfg); err == nil && c != nil {
		objectKey := artifactObjectName(ownerID, key)
...
func deleteUserArtifacts(ctx context.Context, cfg Config, ownerID string) error {
	if c, err := objectStoreClientFor(cfg); err == nil && c != nil {
		return c.DeletePrefix(ctx, artifactsBucketName, artifactBasePrefix(ownerID))
	} else if err != nil {
		log.Printf("object store client unavailable (falling back to encore objects): %v", err)
	}

	prefix := artifactBasePrefix(ownerID)
