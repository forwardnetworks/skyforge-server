...
func (s *Service) ListUserDeploymentUIEvents(ctx context.Context, id, deploymentID string, params *ListDeploymentUIEventsParams) (*ListDeploymentUIEventsResponse, error) {
	...
	defer cancel()
	rows, err := listDeploymentUIEventsAfter(ctxQ, s.db, pc.context.ID, deploymentID, after, limit)
	if err != nil {
...
func (s *Service) DeploymentUIEventsStream(w http.ResponseWriter, req *http.Request) {
	...
	if ownerKey == "" || deploymentID == "" {
		...
		// expected:
		// - api/users/me/deployments/<deploymentID>/ui-events/events
		// - api/contexts/<id>/deployments/<deploymentID>/ui-events/events
...
func insertDeploymentUIEvent(ctx context.Context, db *sql.DB, ownerID, deploymentID, createdBy, eventType string, payload any) error {
	...
	_, err := db.ExecContext(ctxReq, `
		INSERT INTO sf_deployment_ui_events (owner_username, deployment_id, created_by, event_type, payload)
		VALUES ($1, $2, $3, $4, $5::jsonb)
	`, ownerID, deploymentID, createdBy, eventType, string(raw))
	return err
}

func listDeploymentUIEventsAfter(ctx context.Context, db *sql.DB, ownerID, deploymentID string, afterID int64, limit int) ([]DeploymentUIEvent, error) {
	if db == nil {
		return nil, nil
	}
	...
	rows, err := db.QueryContext(ctxReq, `
		...
		WHERE owner_username=$1 AND deployment_id=$2 AND id > $3
		ORDER BY id ASC
		LIMIT $4
	`, ownerID, deploymentID, afterID, limit)
	if err != nil {
