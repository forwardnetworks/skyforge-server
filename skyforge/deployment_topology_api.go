...
//
//encore:api auth method=GET path=/api/user-contexts/:id/deployments/:deploymentID/topology
...
func (s *Service) getDeploymentTopologyFromLatestTaskArtifact(ctx context.Context, pc *userContext, dep *UserDeployment, taskType string) (*DeploymentTopologyResponse, error) {
	taskType = strings.TrimSpace(taskType)
	if taskType == "" {
		return nil, errs.B().Code(errs.InvalidArgument).Msg("task type is required").Err()
	}
	if task, err := getLatestDeploymentTask(ctx, s.db, pc.userContext.ID, dep.ID, taskType); err == nil && task != nil {
		key := strings.TrimSpace(getJSONMapString(task.Metadata, "topologyKey"))
		if key != "" {
			ctxRead, cancel := context.WithTimeout(ctx, 5*time.Second)
			defer cancel()
			if raw, err := readUserContextArtifact(ctxRead, s.cfg, pc.userContext.ID, key, 2<<20); err == nil && len(raw) > 0 {
...
func (s *Service) getContainerlabDeploymentTopology(ctx context.Context, pc *userContext, dep *UserDeployment) (*DeploymentTopologyResponse, error) {
	...
	if task, err := getLatestDeploymentTask(ctx, s.db, pc.userContext.ID, dep.ID, "containerlab-run"); err == nil && task != nil {
		key := strings.TrimSpace(getJSONMapString(task.Metadata, "topologyKey"))
		if key != "" {
			ctxRead, cancel := context.WithTimeout(ctx, 5*time.Second)
			defer cancel()
