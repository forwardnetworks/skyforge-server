...
func (s *Service) runNetlabC9sDeploymentAction(
	...
	if mode == "remote" {
		...
		server, err := s.resolveUserContextNetlabServerConfig(ctx, pc.userContext.ID, netlabServer)
		if err != nil {
	...
	userContextRoot := fmt.Sprintf("/home/%s/netlab", strings.TrimSpace(pc.claims.Username))
	...
	meta, err := toJSONMap(map[string]any{
		...
		"labName":      labName,
		"dedupeKey":    fmt.Sprintf("netlab-c9s:%s:%s:%s", pc.userContext.ID, action, dep.ID),
	...
	var task *TaskRecord
	if allowActive {
		task, err = createTaskAllowActive(ctx, s.db, pc.userContext.ID, &dep.ID, "netlab-c9s-run", message, pc.claims.Username, meta)
	} else {
		task, err = createTask(ctx, s.db, pc.userContext.ID, &dep.ID, "netlab-c9s-run", message, pc.claims.Username, meta)
	}
	if err != nil {
		return nil, err
	}
