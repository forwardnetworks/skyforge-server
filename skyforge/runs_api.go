...
func (s *Service) GetRuns(ctx context.Context, params *RunsListParams) (*RunsListResponse, error) {
	...
	for _, task := range tasks {
		...
		run["userContextId"] = userContext.ID
...
func (s *Service) CreateRun(ctx context.Context, req *RunRequest) (*RunsCreateResponse, error) {
	...
	if !isAdminUser(s.cfg, user.Username) {
		return nil, errs.B().Code(errs.PermissionDenied).Msg("forbidden (use user-context run endpoints)").Err()
	}
	if req == nil {
...
func (s *Service) CancelRun(ctx context.Context, id int, params *RunsOutputParams) (*RunsCancelResponse, error) {
	...
	// Load task first to determine user-context/deployment ownership.
	...
	// Resolve access against the task owner context.
	userContext, err := s.resolveUserContextForUser(ctx, user, task.WorkspaceID)
	if err != nil {
		return nil, err
	}
	if userContext.ID != task.WorkspaceID {
		return nil, errs.B().Code(errs.PermissionDenied).Msg("forbidden").Err()
	}
	// Viewers can see runs but shouldn't cancel them.
	pc, err := s.userContextForUser(user, userContext.ID)
	if err != nil {
		return nil, err
	}
	if pc.access == "viewer" {
	...
	if task.DeploymentID.Valid {
		...
		userContextID := strings.TrimSpace(task.WorkspaceID)
		deploymentID := strings.TrimSpace(task.DeploymentID.String)
		if userContextID != "" && deploymentID != "" {
			if nextID, err := getOldestQueuedDeploymentTaskID(ctx, s.db, userContextID, deploymentID); err == nil && nextID > 0 {
				ctxKick, cancel := context.WithTimeout(context.Background(), 2*time.Second)
				s.enqueueTaskID(ctxKick, nextID, userContextID, deploymentID, 0)
				cancel()
			}
		}
	}
	...
	if err != nil {
		return &RunsCancelResponse{TaskID: task.ID, UserContextID: userContext.ID, Status: "canceled", User: user.Username}, nil
	}
	return &RunsCancelResponse{TaskID: task.ID, UserContextID: userContext.ID, Status: "canceled", Task: &taskJSON, User: user.Username}, nil
}
