...
func createPolicyReportForwardNetwork(ctx context.Context, db *sql.DB, ownerID string, actor string, req *PolicyReportCreateForwardNetworkRequest) (*PolicyReportForwardNetwork, error) {
	if db == nil {
...
INSERT INTO sf_policy_report_forward_networks (
  id, owner_username, forward_network_id, name, description, collector_config_id, created_by
) VALUES ($1,$2,$3,$4,NULLIF($5,''),NULLIF($6,''),$7)
`, id, ownerID, forwardID, name, desc, collectorConfigID, actor)
	if err != nil {
		return nil, err
	}
...
INSERT INTO sf_policy_report_forward_networks (
  id, owner_username, owner_username, forward_network_id, name, description, collector_config_id, created_by
)
...
func listPolicyReportForwardNetworks(ctx context.Context, db *sql.DB, ownerID string) ([]PolicyReportForwardNetwork, error) {
	if db == nil {
		return nil, fmt.Errorf("db is not configured")
	}
	ctx, cancel := context.WithTimeout(ctx, 3*time.Second)
...
SELECT id, owner_username, forward_network_id, name, COALESCE(description,''), COALESCE(collector_config_id,''), created_by, created_at, updated_at
 ...
 ORDER BY created_at DESC`, ownerID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&n.ID, &n.OwnerUsername, &n.ForwardNetwork, &n.Name, &desc, &collectorConfigID, &n.CreatedBy, &n.CreatedAt, &n.UpdatedAt); err != nil {
			return nil, err
		}
...
func listUserPolicyReportForwardNetworks(ctx context.Context, db *sql.DB, ownerUsername string) ([]PolicyReportForwardNetwork, error) {
	...
	rows, err := db.QueryContext(ctx, `
SELECT id, COALESCE(owner_username,''), forward_network_id, name, COALESCE(description,''), COALESCE(collector_config_id,''), created_by, created_at, updated_at
  FROM sf_policy_report_forward_networks
 WHERE owner_username=$1
...
func deletePolicyReportForwardNetwork(ctx context.Context, db *sql.DB, ownerID string, forwardNetworkRef string) error {
	if db == nil {
