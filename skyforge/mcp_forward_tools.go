...
func (s *Service) mcpForwardToolsCall(ctx context.Context, user *AuthUser, ownerID, forwardNetworkID, name string, args map[string]any) (string, error) {
	if user == nil {
	...
	rec, err := resolveForwardCredentialsFor(ctx, s.db, s.cfg.SessionSecret, ownerID, user.Username, forwardNetworkID, forwardCredResolveOpts{
	...
	case "get_device_basic_info":
		return s.mcpForwardToolsCall(ctx, user, ownerID, forwardNetworkID, "run_nqe_query_by_id", withArgs(args, map[string]any{"query_id": "FQ_ac651cb2901b067fe7dbfb511613ab44776d8029"}))
	case "get_device_hardware":
		return s.mcpForwardToolsCall(ctx, user, ownerID, forwardNetworkID, "run_nqe_query_by_id", withArgs(args, map[string]any{"query_id": "FQ_7ec4a8148b48a91271f342c512b2af1cdb276744"}))
	case "get_hardware_support":
		return s.mcpForwardToolsCall(ctx, user, ownerID, forwardNetworkID, "run_nqe_query_by_id", withArgs(args, map[string]any{"query_id": "FQ_f0984b777b940b4376ed3ec4317ad47437426e7c"}))
	case "get_os_support":
		return s.mcpForwardToolsCall(ctx, user, ownerID, forwardNetworkID, "run_nqe_query_by_id", withArgs(args, map[string]any{"query_id": "FQ_fc33d9fd70ba19a18455b0e4d26ca8420003d9cc"}))
	case "search_configs":
		...
		return s.mcpForwardToolsCall(ctx, user, ownerID, forwardNetworkID, "run_nqe_query_by_id", a2)
