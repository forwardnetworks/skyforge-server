...
func listWorkspaceNetlabServers(ctx context.Context, db *sql.DB, box *secretBox, workspaceID string) ([]workspaceNetlabServer, error) {
	...
	rows, err := db.QueryContext(ctx, `SELECT id, project_id, name, api_url, api_insecure, COALESCE(api_token,''), created_at, updated_at
FROM sf_project_netlab_servers WHERE project_id=$1 ORDER BY name ASC`, workspaceID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &projectID, &name, &apiURL, &insecure, &tokenEnc, &createdAt, &updatedAt); err != nil {
			return nil, err
		}
...
func getWorkspaceNetlabServerByID(ctx context.Context, db *sql.DB, box *secretBox, workspaceID, id string) (*workspaceNetlabServer, error) {
	...
	var tokenEnc string
	err := db.QueryRowContext(ctx, `SELECT id, project_id, name, api_url, api_insecure, COALESCE(api_token,''), created_at, updated_at
...
func upsertWorkspaceNetlabServer(ctx context.Context, db *sql.DB, box *secretBox, rec workspaceNetlabServer) (*workspaceNetlabServer, error) {
  ...
  id, project_id, name, api_url, api_insecure, api_token, created_at, updated_at
...
ON CONFLICT (id) DO UPDATE SET
  ...
  updated_at=now()`,
		id, rec.WorkspaceID, rec.Name, rec.APIURL, rec.APIInsecure, tokenEnc,
	)
	if err != nil {
		return nil, err
	}
...
FROM sf_project_eve_servers WHERE project_id=$1 ORDER BY name ASC`, workspaceID)
	...
	for rows.Next() {
		...
		var webURL string
		var sshHost, sshUser, sshKeyEnc string
		if err := rows.Scan(
			&rec.ID,
			&rec.WorkspaceID,
			&rec.Name,
...
func upsertWorkspaceEveServer(ctx context.Context, db *sql.DB, box *secretBox, rec workspaceEveServer) (*workspaceEveServer, error) {
  ...
  id, project_id, name, api_url, web_url, skip_tls_verify, ssh_host, ssh_user, ssh_key, created_at, updated_at
...
ON CONFLICT (id) DO UPDATE SET
  ...
  updated_at=now()`,
		id, rec.WorkspaceID, rec.Name, rec.APIURL, rec.WebURL, rec.SkipTLSVerify, rec.SSHHost, rec.SSHUser, sshKeyEnc,
	)
	if err != nil {
		return nil, err
	}
