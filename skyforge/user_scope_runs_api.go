...
type UserScopeRunResponse struct {
	UserScopeID string  `json:"userId"`
	Task        JSONMap `json:"task"`
	User        string  `json:"user"`
}

type UserScopeTerraformApplyParams struct {
...
func (s *Service) RunUserScopeTerraformPlan(ctx context.Context, id string) (*UserScopeRunResponse, error) {
	...
	dep, err := s.getLatestDeploymentByType(ctx, pc.userScope.ID, "terraform")
	if err != nil {
		return nil, err
	}
	if dep == nil {
	...
	region := strings.TrimSpace(pc.userScope.AWSRegion)
	if region == "" {
	...
	if s.cfg.UserScopes.ObjectStorageAccessKey != "" && s.cfg.UserScopes.ObjectStorageSecretKey != "" {
		env["AWS_ACCESS_KEY_ID"] = s.cfg.UserScopes.ObjectStorageAccessKey
		env["AWS_SECRET_ACCESS_KEY"] = s.cfg.UserScopes.ObjectStorageSecretKey
	}
	if shouldUseAWS(pc.userScope) {
		if strings.TrimSpace(pc.userScope.AWSAuthMethod) == "" {
			pc.userScope.AWSAuthMethod = "sso"
		}
		env["TF_VAR_aws_region"] = region
		if err := populateAWSAuthEnv(ctx, s.cfg, s.db, s.awsStore, pc.userScope, pc.claims.Username, env); err != nil {
			return nil, err
		}
	}
	if err := populateAzureAuthEnv(ctx, s.cfg, s.db, pc.userScope, env); err != nil {
		return nil, err
	}
	if err := populateGCPAuthEnv(ctx, s.cfg, s.db, pc.userScope, env); err != nil {
		return nil, err
	}
	...
	deploymentEnv, err := s.mergeDeploymentEnvironment(ctx, pc.userScope.ID, user.Username, cfgAny)
	if err != nil {
		return nil, err
	}
	...
	task, err := createTask(ctx, s.db, pc.userScope.ID, &dep.ID, "terraform-plan", fmt.Sprintf("Skyforge terraform plan (%s)", pc.claims.Username), pc.claims.Username, meta)
	if err != nil {
		return nil, err
	}
	s.queueTask(task)
...
func (s *Service) RunUserScopeTerraformApply(ctx context.Context, id string, params *UserScopeTerraformApplyParams) (*UserScopeRunResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	if cloud == "aws" && shouldUseAWS(pc.userScope) {
		if strings.TrimSpace(pc.userScope.AWSAuthMethod) == "" {
			pc.userScope.AWSAuthMethod = "sso"
		}
		env["TF_VAR_aws_region"] = region
	...
	envMap := env
	{
		...
		writeAuditEvent(
			...
			actorIsAdmin,
			impersonated,
			"user_scope.run.terraform-apply",
			pc.userScope.ID,
	...
	task, err := createTask(ctx, s.db, pc.userScope.ID, nil, fmt.Sprintf("terraform-%s", action), fmt.Sprintf("Skyforge terraform %s %s (%s)", action, strings.ToUpper(cloud), pc.claims.Username), pc.claims.Username, meta)
	if err != nil {
		return nil, err
	}
	s.queueTask(task)
...
func (s *Service) RunUserScopeAnsible(ctx context.Context, id string) (*UserScopeRunResponse, error) {
	_ = ctx
	_ = id
	return nil, errs.B().Code(errs.Unimplemented).Msg("ansible runs are not supported in native mode").Err()
}

type UserScopeNetlabRunRequest struct {
...
type UserScopeContainerlabRunRequest struct {
	Message        string  `json:"message,omitempty"`
	GitBranch      string  `json:"gitBranch,omitempty"`
	Environment    JSONMap `json:"environment,omitempty"`
...
type UserScopeEveNgRunRequest struct {
	Message        string `json:"message,omitempty"`
...
func (s *Service) RunUserScopeNetlab(ctx context.Context, id string, req *UserScopeNetlabRunRequest) (*UserScopeRunResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	if topologyPath == "" && topologyURL == "" {
		if templateName != "" {
			...
			case "blueprints", "blueprint":
				...
				branch = ""
			case "user":
				if !pc.userScope.IsPublic {
					return nil, errs.B().Code(errs.FailedPrecondition).Msg("user-scope repo is private; netlab BYOS requires a public topologyUrl (use the public blueprints repo or make the repo public)").Err()
				}
	...
	meta, err := toJSONMap(map[string]any{
		...
		"dedupeKey":  fmt.Sprintf("netlab:%s:%s:%s", pc.userScope.ID, action, deploymentName),
		"spec": map[string]any{
			"action":        action,
			"server":        serverRef,
			...
			"userScopeDir":  strings.TrimSpace(userScopeDir),
	...
	var task *TaskRecord
	if allowActive {
		task, err = createTaskAllowActive(ctx, s.db, pc.userScope.ID, nil, "netlab-run", message, pc.claims.Username, meta)
	} else {
		task, err = createTask(ctx, s.db, pc.userScope.ID, nil, "netlab-run", message, pc.claims.Username, meta)
	}
	if err != nil {
		return nil, err
	}
...
func (s *Service) RunUserScopeContainerlab(ctx context.Context, id string, req *UserScopeContainerlabRunRequest) (*UserScopeRunResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	if req == nil {
		req = &UserScopeContainerlabRunRequest{}
	}
	...
	if action == "deploy" {
		...
		// For private user-scope repos, the BYOS host might not be able to fetch the raw URL, so we keep a
		...
		if templateSource == "external" {
			...
			shouldUseSource := templateSource != "user" || pc.userScope.IsPublic
			if shouldUseSource {
				topologySourceURL = giteaRawFileURL(s.cfg, ref.Owner, ref.Repo, ref.Branch, filePath)
			} else {
	...
	meta, err := toJSONMap(map[string]any{
		...
		"template":  template,
		"priority":  taskPriorityInteractive,
		"dedupeKey": fmt.Sprintf("containerlab:%s:%s:%s", pc.userScope.ID, action, labName),
		"spec": map[string]any{
			"action":            action,
	...
	var task *TaskRecord
	if allowActive {
		task, err = createTaskAllowActive(ctx, s.db, pc.userScope.ID, nil, "containerlab-run", message, pc.claims.Username, meta)
	} else {
		task, err = createTask(ctx, s.db, pc.userScope.ID, nil, "containerlab-run", message, pc.claims.Username, meta)
	}
	if err != nil {
		return nil, err
	}
...
func (s *Service) RunUserScopeEveNg(ctx context.Context, id string, req *UserScopeEveNgRunRequest) (*UserScopeRunResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	if req == nil {
		req = &UserScopeEveNgRunRequest{}
	}

	serverRef := strings.TrimSpace(req.EveServer)
	if serverRef == "" {
		serverRef = strings.TrimSpace(pc.userScope.EveServer)
	}
	...
	if labPath == "" {
		labPath = path.Join("skyforge", pc.userScope.Slug, deploymentName+".unl")
	}
	...
	meta, err := toJSONMap(map[string]any{
		...
		"template":  template,
		"priority":  taskPriorityInteractive,
		"dedupeKey": fmt.Sprintf("eve-ng:%s:%s:%s", pc.userScope.ID, action, labPath),
		"spec": map[string]any{
			"action":         action,
			"server":         serverRef,
			"serverLabel":    strings.TrimSpace(server.Name),
	...
	var task *TaskRecord
	if allowActive {
		task, err = createTaskAllowActive(ctx, s.db, pc.userScope.ID, nil, "eve-ng-run", message, pc.claims.Username, meta)
	} else {
		task, err = createTask(ctx, s.db, pc.userScope.ID, nil, "eve-ng-run", message, pc.claims.Username, meta)
	}
	if err != nil {
		return nil, err
	}
...
func populateAWSAuthEnv(ctx context.Context, cfg Config, db *sql.DB, store awsSSOTokenStore, userScope UserScope, username string, env map[string]any) error {
	switch strings.ToLower(strings.TrimSpace(userScope.AWSAuthMethod)) {
	case "sso":
		accountID := strings.TrimSpace(userScope.AWSAccountID)
		roleName := strings.TrimSpace(userScope.AWSRoleName)
		if accountID == "" || roleName == "" {
			return errs.B().Code(errs.InvalidArgument).Msg("user scope is missing awsAccountId/awsRoleName").Err()
		}
	...
	case "static":
		...
		if rec == nil || rec.AccessKeyID == "" || rec.SecretAccessKey == "" {
			return errs.B().Code(errs.InvalidArgument).Msg("aws static credentials are not configured for this user scope").Err()
		}
...
func shouldUseAWS(userScope UserScope) bool {
	authMethod := strings.ToLower(strings.TrimSpace(userScope.AWSAuthMethod))
	if authMethod == "" {
		authMethod = "sso"
	}
	switch authMethod {
	case "static":
		return true
	case "sso":
		return strings.TrimSpace(userScope.AWSAccountID) != "" && strings.TrimSpace(userScope.AWSRoleName) != ""
	default:
		return false
	}
}

func populateAzureAuthEnv(ctx context.Context, cfg Config, db *sql.DB, userScope UserScope, env map[string]any) error {
	if db == nil {
		return nil
	}
	ctx, cancel := context.WithTimeout(ctx, 2*time.Second)
...
func populateGCPAuthEnv(ctx context.Context, cfg Config, db *sql.DB, userScope UserScope, env map[string]any) error {
	if db == nil {
		return nil
	}
	ctx, cancel := context.WithTimeout(ctx, 2*time.Second)
	...
	providerProjectID := strings.TrimSpace(rec.ProjectIDOverride)
	...
	if providerProjectID != "" {
		env["TF_VAR_gcp_project"] = providerProjectID
		env["GOOGLE_PROJECT"] = providerProjectID
	}
