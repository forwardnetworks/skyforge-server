...
type UserScopeNetlabTemplatesResponse struct {
...
type UserScopeNetlabTemplatesRequest struct {
...
type UserScopeNetlabValidateRequest struct {
...
func (s *Service) GetUserScopeNetlabTemplates(ctx context.Context, id string, req *UserScopeNetlabTemplatesRequest) (*UserScopeNetlabTemplatesResponse, error) {
	...
	if cached, err := loadTemplateIndex(ctx, s.db, "netlab", owner, repo, branch, dir); err == nil && cached != nil {
		if headSHA != "" && strings.TrimSpace(cached.HeadSHA) != "" && cached.HeadSHA == headSHA {
			...
			return &UserScopeNetlabTemplatesResponse{
...
// ValidateUserScopeNetlabTemplate runs `netlab create` against a selected template bundle without deploying it.
...
func (s *Service) ValidateUserScopeNetlabTemplate(ctx context.Context, id string, req *UserScopeNetlabValidateRequest) (*UserScopeRunResponse, error) {
	user, err := requireAuthUser()
	if err != nil {
		return nil, err
	}
	...
	task, err := createTask(ctx, s.db, pc.userScope.ID, nil, "netlab-validate", "Skyforge netlab validate", pc.claims.Username, meta)
	if err != nil {
		return nil, err
	}
	s.queueTask(task)
...
func isNetlabTemplatePathExcluded(rel string) bool {
	...
	//
	// These commonly appear when a folder is synced from a runner user scope rather than a
...
func listNetlabTemplatesViaGitTree(cfg Config, owner, repo, headSHA, dir string, maxResults int) ([]string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		fullURL := strings.TrimRight(cfg.UserScopes.GiteaAPIURL, "/") + apiPath
