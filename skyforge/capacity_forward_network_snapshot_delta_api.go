...
// GetUserContextForwardNetworkCapacitySnapshotDelta compares the last two processed snapshot inventories.
//
// This is a lightweight "changes tab" view: route scale + BGP scale deltas, plus basic inventory drift.
func (s *Service) GetUserContextForwardNetworkCapacitySnapshotDelta(ctx context.Context, id, networkRef string) (*ForwardNetworkCapacitySnapshotDeltaResponse, error) {
	...
	latestSID, prevSID, err := loadLatestTwoCapacitySnapshotIDs(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID)
	if err != nil {
	...
	latestRoutes, _ := loadCapacityRouteScaleSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, latestSID)
	prevRoutes, _ := loadCapacityRouteScaleSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, prevSID)
	resp.RouteDelta = diffRouteScale(prevRoutes, latestRoutes)

	latestBgp, _ := loadCapacityBgpNeighborsSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, latestSID)
	prevBgp, _ := loadCapacityBgpNeighborsSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, prevSID)
	resp.BgpDelta = diffBgpScale(prevBgp, latestBgp)

	latestDevices, _ := loadCapacityDevicesSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, latestSID)
	prevDevices, _ := loadCapacityDevicesSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, prevSID)
	resp.DeviceDelta = diffDevices(prevDevices, latestDevices)

	latestIfaces, _ := loadCapacityInterfacesSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, latestSID)
	prevIfaces, _ := loadCapacityInterfacesSnapshot(ctx, s.db, pc.userContext.ID, net.ForwardNetworkID, prevSID)
	resp.InterfaceDelta = diffInterfaces(prevIfaces, latestIfaces)

	return resp, nil
}

func loadLatestTwoCapacitySnapshotIDs(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID string) (latestSID string, prevSID string, err error) {
	if db == nil {
...
ORDER BY as_of DESC
LIMIT 2`, userContextID, forwardNetworkID)
	if err != nil {
		return "", "", err
	}
	defer rows.Close()
...
func loadCapacityNQESnapshotPayload(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, queryID, snapshotID string) (payload []byte, err error) {
	if db == nil {
	...
	if userContextID == "" || forwardNetworkID == "" || queryID == "" || snapshotID == "" {
...
LIMIT 1`, userContextID, forwardNetworkID, queryID, snapshotID).Scan(&b); err != nil {
...
func loadCapacityRouteScaleSnapshot(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, snapshotID string) ([]CapacityRouteScaleRow, error) {
	b, err := loadCapacityNQESnapshotPayload(ctx, db, userContextID, forwardNetworkID, "capacity-route-scale.nqe", snapshotID)
	if err != nil || len(b) == 0 {
...
func loadCapacityBgpNeighborsSnapshot(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, snapshotID string) ([]CapacityBgpNeighborRow, error) {
	b, err := loadCapacityNQESnapshotPayload(ctx, db, userContextID, forwardNetworkID, "capacity-bgp-neighbors.nqe", snapshotID)
	if err != nil || len(b) == 0 {
...
func loadCapacityDevicesSnapshot(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, snapshotID string) ([]CapacityDeviceInventoryRow, error) {
	b, err := loadCapacityNQESnapshotPayload(ctx, db, userContextID, forwardNetworkID, "capacity-devices.nqe", snapshotID)
	if err != nil || len(b) == 0 {
...
func loadCapacityInterfacesSnapshot(ctx context.Context, db *sql.DB, userContextID, forwardNetworkID, snapshotID string) ([]CapacityInterfaceInventoryRow, error) {
	b, err := loadCapacityNQESnapshotPayload(ctx, db, userContextID, forwardNetworkID, "capacity-interfaces.nqe", snapshotID)
	if err != nil || len(b) == 0 {
