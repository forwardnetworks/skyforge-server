...
//
//encore:api auth method=POST path=/api/user-contexts/:id/deployments/:deploymentID/links/admin
func (s *Service) UpdateUserDeploymentLinkAdmin(ctx context.Context, id, deploymentID string, req *DeploymentLinkAdminRequest) (*DeploymentLinkAdminResponse, error) {
	...
	if s.db != nil {
		...
		if err := insertDeploymentUIEvent(ctx, s.db, pc.userContext.ID, deploymentID, pc.claims.Username, "link."+action, payload); err == nil {
			_ = notifyDeploymentEventPG(ctx, s.db, pc.userContext.ID, deploymentID)
		}
	}
...
// CaptureWorkspaceDeploymentLinkPcap captures a short pcap on a topology link and uploads it as a user-context artifact.
//
//encore:api auth method=POST path=/api/user-contexts/:id/deployments/:deploymentID/links/capture
func (s *Service) CaptureWorkspaceDeploymentLinkPcap(ctx context.Context, id, deploymentID string, req *DeploymentLinkCaptureRequest) (*DeploymentLinkCaptureResponse, error) {
	...
	if err := storageSvc.Write(ctx, &storage.WriteRequest{ObjectName: artifactObjectName(pc.userContext.ID, key), Data: payload}); err != nil {
	...
	if s.db != nil {
		...
		if err := insertDeploymentUIEvent(ctx, s.db, pc.userContext.ID, deploymentID, pc.claims.Username, "link.capture", payloadEv); err == nil {
			_ = notifyDeploymentEventPG(ctx, s.db, pc.userContext.ID, deploymentID)
		}
	}
